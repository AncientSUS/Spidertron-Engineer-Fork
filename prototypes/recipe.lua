if not mods["space-exploration"] then
  ...
  if settings.startup["spidertron-engineer-space-science-to-fish"].value then
    local fish = {
        ...
        order = "y",
        ingredients = {{type = "item", name = "space-science-pack", amount = 10}},
        icon = "__base__/graphics/icons/fish.png",
        icon_size = 64, icon_mipmaps = 4,

        results = {{type = "item", name = "raw-fish", amount = 1, probability = 0.05}},
        enabled = false,
  ...
  if settings.startup["spidertron-engineer-rocket-returns-fish"].value then
    ...
    if spidertron_in_rocket then
      ...
      data.raw["item-with-entity-data"]["spidertron"]["rocket_launch_products"] = {{type = "item", name = "space-science-pack", amount = 1000}, {type = "item", name = "raw-fish", amount = 5, probability = 0.1}, {type = "item", name = "raw-fish", amount = 10, probability = 0.05}}
    else
      data.raw["item-with-entity-data"]["satellite"]["rocket_launch_product"] = nil
      data.raw["item-with-entity-data"]["satellite"]["rocket_launch_products"] = {{type = "item", name = "space-science-pack", amount = 1000}, {type = "item", name = "raw-fish", amount = 1, probability = 0.5}}
    end
  end
end